"""Direct Keras to TFJS Exporter untuk Model v5 Champion.
...
def export_model_to_tfjs(model: keras.Model, out_dir: Path):
    ...
    model_json = {
        "format": "layers-model",
        "generatedBy": "Keras 3 Champion Exporter",
    ...
    print(f"Export Model v5 Champion sukses: {model_json_path} ({shard_path.stat().st_size} bytes)")


if __name__ == "__main__":
    base_dir = Path(r"d:\Penyimpanan Utama\Documents\python\IsyaratLive")
    model_path = base_dir / "ml/checkpoints/v5/best.keras"
    model = keras.models.load_model(model_path, safe_mode=False)
    print("Loaded model successfully via safe_mode=False!")
    export_model_to_tfjs(
        model,
        base_dir / "frontend/public/models/gloss-classifier-v5"
    )
